...
import "./index.less";
import { checkOneToManyNode, oneToOneNode } from "./config";

// console.log("start=", start);

const spaceYPoint = 360;
const spaceXPoint = 360;
    ...
    div.addEventListener(
      ...
      function (event) {
        ...
        if (this === _this.myDiagram.div && _this.data.highlightedData) {
          ...
          // 一对一进行添加节点
          const nodeInfo = oneToOneNode(originNodeData, newNodeData);
          console.log("节点----->", nodeInfo);
          const showLinkData = [...linkDataArray];

          const { insertIndex, count, firstNode } = nodeInfo;

          // 处理节点数据
          Object.assign(newNodeData, {
            xy: `${+parentX} ${firstNode ? spaceYPoint : spaceYPoint}`,
          });

          originNodeData.map((item, index) => {
            if (index >= insertIndex) {
              const xyList = item.xy.split(" ");
              const x = +xyList[0];
              const y = +xyList[1];
              Object.assign(item, {
                xy: `${x} ${y + spaceYPoint}`,
                parentKey:
                  index === insertIndex ? newNodeData.key : item.parentKey,
              });
            }
            return item;
          });
          originNodeData.splice(insertIndex, 0, newNodeData);

          if (firstNode) {
            // 第一个子节点
            // 处理连线数据
            showLinkData[parentLinkIndex].to = newNodeData.key;
            const link = {
              key: _this.getUuid(),
              ...
              points: [
                ...
                +parentY + spaceYPoint,
                +parentX + 38,
                +parentY + spaceYPoint * 2 + 200,
                +parentX + 38,
                +parentY + spaceYPoint * 2 + 200,
              ],
            };
            showLinkData.push(link);
          } else {
            //
            showLinkData.map((item, index) => {
              if (index >= insertIndex) {
                // Object.assign(item, {
                //   from: index === insertIndex ? newNodeData.key : item.from,
                // });
                item.points[1] = item.points[1] + spaceYPoint * 2;
                item.points[3] = item.points[1] + spaceYPoint * 2;
                item.points[5] = item.points[1] + spaceYPoint * 2;
              }
              return item;
            });
            ...
            const link = {
              key: _this.getUuid(),
              from: newNodeData.key,
              to: showLinkData[parentLinkIndex].to,
              points: [
                +parentX + 38,
                +parentY + spaceYPoint,
                +parentX + 38,
                +parentY + spaceYPoint * 2,
                +parentX + 38,
                +parentY + spaceYPoint * 2,
              ],
            };

            showLinkData.splice(insertIndex, 0, link);
            // 处理连线数据
            showLinkData[parentLinkIndex].to = newNodeData.key;
          }

          console.log("节点---->", originNodeData, showLinkData);
          _this.setState({
            nodeDataArray: originNodeData,
            originNodeData,
            linkDataArray: showLinkData,
          });
