...
import { checkOneToManyNode } from "./config.js";
import "./index.less";

const nodeXSpace = 240;
const nodeYSpace = 240;
    ...
    div.addEventListener(
      ...
      function (event) {
        ...
        if (this === _this.myDiagram.div && _this.data.highlightedData) {
          ...
          //   console.log("nodedata item---->", item);
          ...
          const newNodeData = {
            ...
            rightText: text,
            icon: src,
            xy: `${+parentX} ${+parentY + nodeYSpace}`,
          };

          const showLinkData = [...linkDataArray];
          const showNodeData = [...nodeDataArray];

          // 检测节点一对多关系
          const nodeInfo = checkOneToManyNode(nodeDataArray, newNodeData);
          if (nodeInfo.oneToMany) {
            // 一对多
            const { count } = nodeInfo;
            // debugger;
            // 节点
            Object.assign(parentNodeData, {
              xy: `${(nodeXSpace * count) / 2} ${+parentY}`,
            });

            Object.assign(newNodeData, {
              xy: `${nodeXSpace * count} ${+parentY + nodeYSpace}`,
            });

            showNodeData.splice(nodeInfo.insertIndex, 0, newNodeData);
            console.log("节点数据------>", showNodeData, nodeInfo.insertIndex);

            // 连线
            const oneToManyLink = {
              key: _this.getUuid(),
              from: parentNodeData.key,
              to: key,
              points: [
                +parentX + nodeYSpace,
                +parentY + nodeYSpace + 38,
                +parentX,
                +parentY + nodeYSpace + 190,
                +parentX + nodeYSpace,
                +parentY + nodeYSpace + 200,
              ],
            };
            showLinkData.splice(nodeInfo.insertIndex, 0, oneToManyLink);
            ...
            showLinkData.splice(nodeInfo.insertIndex + 1, 0, newLinkDataArray);
          } else {
            // 一对一
            ...
            showNodeData.push(newNodeData);
            showLinkData.push(newLinkDataArray);
          }
