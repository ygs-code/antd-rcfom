 ...
 * @LastEditTime: 2021-06-24 16:58:12
  ...
  componentDidMount() {
    ...
    function init() {
      ...
      myDiagram.nodeTemplate = $(
        ...
        _this.makePort(go.Spot.Top, false, true),
        _this.makePort(go.Spot.Left, true, true),
        _this.makePort(go.Spot.Right, true, true),
        _this.makePort(go.Spot.Bottom, true, false),
      ...
      load(); // load an initial diagram from some JSON text //从JSON文本加载一个初始图表

      // initialize the Palette that is on the left side of the page 初始化页面左侧的Palette
      // 面板
      // let myPalette = $(
      //   go.Palette,
      //   "myPaletteDiv", // must name or refer to the DIV HTML element //必须命名或引用DIV HTML元素
      //   {
      //     maxSelectionCount: 1,
      //     nodeTemplateMap: myDiagram.nodeTemplateMap, // share the templates used by myDiagram //共享myDiagram使用的模板
      //     // simplify the link template, just in this Palette //简化链接模板，就在这个面板中
      //     linkTemplate: $(
      //       go.Link,
      //       {
      //         // because the GridLayout.alignment is Location and the nodes have locationSpot == Spot.Center,
      //         // to line up the Link in the same manner we have to pretend the Link has the same location spot
      //         //因为GridLayout。对齐是位置，节点有locationSpot == Spot。中心,
      //         //以同样的方式排列链接，我们必须假装链接有相同的位置点
      //         locationSpot: go.Spot.Center,
      ...
      //       [
      //         // specify the contents of the Palette 指定Palette的内容
      //         {
      //           text: "Start",
      //           figure: "Ellipse",
      //           size: "75 75",
      ...
      //       ],
      //       [
      //         // the Palette also has a disconnected Link, which the user can drag-and-drop //面板也有一个断开的链接，用户可以拖放
      //         {
    ...
    // Show the diagram's model in JSON format that the user may edit 以JSON格式显示用户可以编辑的图表模型
    // 保存
    function save() {
      saveDiagramProperties(); // do this first, before writing to JSON 在写JSON之前先做这个
